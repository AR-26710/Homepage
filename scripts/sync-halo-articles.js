...
async function fetchHaloPosts(apiUrl, articlesPerCategory, urlConfig) {
  ...
  data.items.forEach((post) => {
    ...
    const article = {
      ...
      url: articleUrl,
      time: post.spec.publishTime ? new Date(post.spec.publishTime).toLocaleDateString('zh-CN') : ''
    };
...
async function main() {
  try {
    ...
    execSync(`git add ${ARTICLES_FILE_PATH}`, { stdio: 'inherit' });
    execSync('git commit -m "chore: 自动同步 Halo 文章"', { stdio: 'inherit' });
    console.log('更改已提交');
  } catch (error) {
